...
# QA：
# 1 如果是48个广告主，那是不是要run 48次？每次run一个广告主？
# 2 paper方法效果不佳，如何调优，有没有trick？
def run_test():
    """
    offline evaluation
    """
    ...
    # load数据集：第7个周期共48个广告主
    # keys：<时间片,adv> ，test_dict：参竞数据info
    keys, test_dict = data_loader.keys, data_loader.test_dict
    # 模拟：假设其他bidder固定，只通过当前agent的bid进行参竞
    # 取当前第一个广告主（第7个周期 && 第0个adv），获取参竞数据，共48个时间片的参竞数据
    key = keys[0]
    num_timeStepIndex, pValues, pValueSigmas, leastWinningCosts = data_loader.mock_data(key)
    rewards = np.zeros(num_timeStepIndex)
    print(f"num_timeStepIndex={num_timeStepIndex}")
    ...
    # 模拟MDP过程，同自动出价训练：积累历史调价日志追加 T-1 -> T
        # state：pValue、Bid、Cost、竞得流量、竞得价格
