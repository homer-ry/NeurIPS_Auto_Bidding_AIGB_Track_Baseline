...
class OfflineEnv:
    ...
    def simulate_ad_bidding(self, pValues: np.ndarray, pValueSigmas: np.ndarray, bids: np.ndarray, leastWinningCosts: np.ndarray, rng=None):
        """
        ...
        values = rng.normal(loc=pValues, scale=pValueSigmas)
        values = values*tick_status
        tick_value = np.clip(values,0,1)
        tick_conversion = rng.binomial(n=1, p=tick_value)
