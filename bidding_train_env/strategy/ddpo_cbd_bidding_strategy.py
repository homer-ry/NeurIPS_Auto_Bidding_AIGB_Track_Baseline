...
class DDPOCbdBiddingStrategy(BaseBiddingStrategy):
    """
    DDPO-CBD Bidding Strategy
    
    Fine-tuned CBD model using DDPO (Denoising Diffusion Policy Optimization)
    """

    def __init__(
        self,
        budget=100,
        name="DDPO-CBD-Bidding-Strategy",
        cpa=2,
        category=1,
        model_name=None,
        model_param=None,
        ...
        if model_name is not None:
            ...
            model_candidates = [
                os.path.join(dir_name, "saved_model", "DDPO-CBD", "diffuser_best.pt"),
                os.path.join(dir_name, "saved_model", "DDPO-CBD", "diffuser.pt"),
                os.path.join(dir_name, "saved_model", "DDPO-CBD-test", "diffuser_best.pt"),
                os.path.join(dir_name, "saved_model", "DDPO-CBD-test", "diffuser.pt"),
            ]

        model_path = next((path for path in model_candidates if os.path.exists(path)), None)
        if model_path is None:
            raise FileNotFoundError(
                "No DDPO-CBD checkpoint found. Checked: "
        ...
        # Load DDPO fine-tuned diffuser checkpoint
    ...
    def bidding(
        ...
        # Use DDPO-enhanced model for action prediction
        actions, _ = self.model(x, rtg)
        alpha = max((actions[0].item() + self.bid_bias) * self.bid_scale, 0)
        bids = alpha * pValues
        return bids
